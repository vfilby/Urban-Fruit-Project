...
$.TokenList = function (input, url_or_data_or_function, settings) {
    ...
    function populate_dropdown (query, results) {
        if( settings.allowCustomEntry ) {
          results.push( { name: query, alias: "Create '" + query + "'" })
        }
        ...
        if(results && results.length) {
            ...
            $.each(results, function(index, value) {
                var token_name;
                if( value.alias ) {
                  token_name = value.alias;
                } else if(settings.parseName) {
                    token_name = settings.parseName(value);
                } else {
                    token_name = value.name;
                }
                
                var this_li = $("<li data-alias=\"" + value.name + "\">" + highlight_term(escapeHTML(token_name), query) + "</li>")
